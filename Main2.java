...
public class Main2 extends Application {
  ...
  private void monthlyReport() throws IOException {
    ...
    List<FarmData> monthlyFarmWeight = new ArrayList<>();
    ...
    // ).collect(Collectors.toList());
    ////////////////
    HashSet<String> uniqueFarmID = new HashSet<String>();
    for (FarmData farm : formatedData) {
      uniqueFarmID.add(farm.getFarmID());
    }

    Hashtable<String, Integer> hashtable = new Hashtable<String, Integer>();
    for (String farmID : uniqueFarmID) {
      hashtable.put(farmID, 0);
    }


    for (FarmData farm : formatedData) {
      String farmID = farm.getFarmID();
      Integer oldWeight = hashtable.get(farmID);
      Integer newWeight = oldWeight + farm.getWeight();
      hashtable.replace(farmID, newWeight);
    }
    ...
    for (String farmID : uniqueFarmID) {
      monthlyFarmWeight.add(new FarmData(farmID, hashtable.get(farmID)));
    }



    setPercent(monthlyFarmWeight);

    table.setItems(FXCollections.observableArrayList(monthlyFarmWeight));
  }

  public static <T> Predicate<T> distinctByFarmId(Function<? super T, ?> keyExtractor) {
